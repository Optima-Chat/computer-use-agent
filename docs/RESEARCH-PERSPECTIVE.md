...
从我们的 benchmark 数据估算（Opus 4.6, 768x576）：

```
一轮 CUA 迭代 (~2-7s)
┌─────────────────────────────────────────────────────────────┐
│  ┌─────────────────────┐                                    │
│  │ 截图捕获 ~50-200ms   │ ← VM 侧 (scrot)                   │
│  │ base64 编码 ~10-50ms │ ← VM 侧                           │
│  └─────────────────────┘                                    │
│           ↓ 网络传输 ~10-50ms                                │
│  ┌─────────────────────┐                                    │
│  │ API 排队 ~100-500ms  │ ← Serving 侧                      │
│  │ 模型推理 ~1-5s       │ ← GPU                             │
│  └─────────────────────┘                                    │
│           ↓ 网络传输 ~10ms                                   │
│  ┌─────────────────────┐                                    │
│  │ 工具执行 ~50-500ms   │ ← VM 侧 (xdotool/bash)            │
│  │ UI 渲染等待 ~200-1s  │ ← VM 侧 (等页面/应用响应)           │
│  └─────────────────────┘                                    │
└─────────────────────────────────────────────────────────────┘
      VM 侧占总延迟 30-50%，推理侧占 50-70%
```

**关键洞察**: VM 侧不是可以忽略的开销。当推理侧继续优化（更快的模型、更好的 batch），VM 侧会成为越来越显著的瓶颈。
